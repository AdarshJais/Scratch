...
export const ThinkBubble = styled.div`
  ...
  &:before {
    ...
    top: -6px;
    left: 14px;
    box-shadow: -25px 15px 0 -6px #fff;
  }
  ...
  &:after {
    ...
    width: 15px;
    height: 15px;
    box-shadow: 20px -17px 0 0 #fff, -14px -3px 0 -1px #fff,
      -12px 8px 0 -3px #fff, -3px 12px 0 -5px #fff;
  }
`;

export const SayBubble = styled.div`
  position: relative;
  display: flex;
  align-items: center;
  justify-content: center;
...
export default function CatSprite({ id }) {
  return (
    <div id={id}>
      ...
      <ThinkBubble id={"think-bubble"} />
      <SayBubble id={"say-bubble"} />
    </div>
  );
}
